((global, factory) => {
  ...
  if (typeof define == 'function' && define.amd) define(['PerfectValidator'], factory)
  global['PerfectValidator'] = factory()
})(this, function () {
  /**
   * 校验器
   * 
   ...
   * @return {function}
   */
  function validator(rule, msg) {
    assert(isFunction(rule),
      'validator rule must be function')
    assert(isString(msg) || isFunction(msg),
   ...
   * @return {function} 
   */
  function asyncValidators(...instances) {
    /**
     * @param {any} value 接收的数据
   ...
   * @return {function}
   */
  function coValidators(...instances) {
    /**
     * @param {any} value 接收的数据
  ...
  function validate(validatorMap) {
    assert(isPlainObject(validatorMap),
      'validatorMap must be plain object')
    ...
    return data => {
      ...
      for (let key in data) {
        ...
        assert(isFunction(keyValidator),
   ...
   * @return {function}
   */
  function asyncValidate(validatorMap) {
    assert(isPlainObject(validatorMap),
      'validatorMap must be plain object')
